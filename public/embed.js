(function () {
  ...
  function render() {
    ...
    setAttrs();
    if (!state.open) {
      root.innerHTML = `<button class="launcher" type="button" aria-label="Open chat"><span class="launcher-icon">${logo("logo") || iconChat("logo")}<span class="launcher-status" aria-hidden="true"></span></span><span class="launcher-copy"><strong>Ask ${esc(shortName(state.config.widgetName))}</strong><span>AI assistant</span></span><span class="launcher-action" aria-hidden="true">${iconArrow()}</span></button>`;
  ...
  function shortName(name) { var clean = String(name || "").trim(); if (!clean) return "Revox"; var short = clean.replace(/\s+(assistant|agent|chat|bot)$/i, "").trim(); return short || clean; }
  ...
  function iconArrow() { return '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M5 12h13M13 6l6 6-6 6" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>'; }
